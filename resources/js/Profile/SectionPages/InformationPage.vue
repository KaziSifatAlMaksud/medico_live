      ...
      <div class="col-12 row">
        <div class="col-md-8">
          <div class="row">
            <div class="form-group">
              ...
              <InputField class="col-md-6" :is-required="true" :label="$t('profile.lbl_first_name')" :value="first_name" :placeholder="$t('profile.lbl_first_name')" 
                v-model="first_name" :error-message="errors['first_name']"></InputField>
            </div>
            <div class="form-group">
              <label class="form-label" for="service">{{ $t('employee.lbl_last_name') }}</label> <span
                class="text-danger">*</span>
              <InputField class="col-md-6" :is-required="true" :label="$t('profile.lbl_last_name')" :value="last_name" :placeholder="$t('profile.lbl_last_name')"
                v-model="last_name" :error-message="errors['last_name']"></InputField>
            </div>
            <div class="form-group">
              <label class="form-label" for="service">{{ $t('employee.lbl_Email') }}</label> <span
                class="text-danger">*</span>
              <InputField class="col-md-6" :is-required="true" :label="$t('profile.lbl_email')" :value="email" :placeholder="$t('profile.lbl_email')"
                v-model="email" :error-message="errors['email']"></InputField>
            </div>
            <div class="form-group col-md-6">
              <label class="form-label"> {{ $t('profile.lbl_contact_number') }} <span class="text-danger">*</span></label>
                <vue-tel-input type="number" :value="mobile" :placeholder="$t('profile.lbl_contact_number')" @input="handleInput" v-bind="{ mode: 'international', maxLen: 15 }"></vue-tel-input>
        ...
        <div class="col-md-4 text-center">
          ...
          <div class="d-flex align-items-center justify-content-center gap-2">
            ...
            <input type="button" class="btn btn-danger" name="remove" :value="$t('settings.remove')" @click="removeLogo()"
              v-if="ImageViewer" />
          </div>
...
const defaultImage = ref('/img/avatar/avatar.webp');
