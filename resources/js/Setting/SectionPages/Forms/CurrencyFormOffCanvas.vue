            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  <div class="col-md-12">
                    <div class="form-group">
                      <label class="form-label">{{ $t('clinic.lbl_currency_name') }} <span
                          class="text-danger">*</span></label>
                      <Multiselect id="curname-list" v-model="currency_name" :value="currency_name"
                        :placeholder="$t('clinic.lbl_currency_name')" v-bind="singleSelectOption"
                        :options="curr_name.options" @select="getSymbol" class="form-group">
                      </Multiselect>
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      ...
                      <InputField :is-required="true" :label="$t('clinic.lbl_symbol')" placeholder=""
                        v-model="currency_symbol" :error-message="errors.currency_symbol"
                        :error-messages="errorMessages['currency_symbol']">
                      </InputField>
                    </div>
                  </div>
                  <div class="col-md-12">
                    <div class="form-group">
                      <label class="form-label">{{ $t('clinic.lbl_currency_code') }} <span
                          class="text-danger">*</span></label>
                      <InputField :is-required="true" :label="$t('clinic.lbl_currency_code')" placeholder=""
                        v-model="currency_code" :error-message="errors.currency_code"
                        :error-messages="errorMessages['currency_code']"></InputField>
                    </div>
                  </div>
                </div>
                <div class="form-group">
                  <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="form-check form-switch">
                      <input class="form-check-input" :value="is_primary" :checked="is_primary" name="is_primary"
                        id="is_primary" type="checkbox" v-model="is_primary" @change="checkPrimary"/>
                    </div>
                  </div>
                </div>
                ...
                <div class="form-group col-md-12">
                  <label class="form-label">{{ $t('currency.lbl_thousand_separatorn') }} <span
                      class="text-danger">*</span></label>
                  <InputField :is-required="true" :label="$t('currency.lbl_thousand_separatorn')" placeholder=""
                    v-model="thousand_separator" :error-message="errors.thousand_separator"
                    :error-messages="errorMessages['thousand_separator']"></InputField>
                </div>
                <div class="form-group col-md-12">
                  <label class="form-label">{{ $t('currency.lbl_decimal_separator') }} <span
                      class="text-danger">*</span></label>
                  <InputField :is-required="true" :label="$t('currency.lbl_decimal_separator')" placeholder=""
                    v-model="decimal_separator" :error-message="errors.decimal_separator"
                    :error-messages="errorMessages['decimal_separator']"></InputField>
                </div>
                <div class="form-group col-md-12">
                  <label class="form-label">{{ $t('currency.lbl_number_of_decimals') }} <span
                      class="text-danger">*</span></label>
                  <InputField :is-required="true" :label="$t('currency.lbl_number_of_decimals')" placeholder=""
                    v-model="no_of_decimal" :error-message="errors.no_of_decimal"
                    :error-messages="errorMessages['no_of_decimal']"></InputField>
                </div>
                <div class="form-group">
                  <label class="form-label" for="label">{{ $t('currency.example') }}:- {{ formatCurrencyVue(labelValue, no_of_decimal,
                    decimal_separator, thousand_separator, currency_position, currency_symbol) }}</label>
                </div>
              </div>
            </div>
          </div>
...
const getCurrName = () => {
  useSelect({ url: COUNTRY_URL }, { value: 'currency_name', label: 'currency_name' }).then((data) => (curr_name.value = data))
}

const getSymbol = async (currencyName) => {
  try {
    const [nameData,symbolData, codeData] = await Promise.all([
      useSelect({ url: COUNTRY_URL, data: { currency_name: currencyName } }, { value: 'currency_name', label: 'currency_name' }),
      useSelect({ url: COUNTRY_URL, data: { currency_name: currencyName } }, { value: 'currency_name', label: 'symbol' }),
      useSelect({ url: COUNTRY_URL, data: { currency_name: currencyName } }, { value: 'currency_name', label: 'currency_code' })
    ]);
