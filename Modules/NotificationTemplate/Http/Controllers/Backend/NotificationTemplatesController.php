...
class NotificationTemplatesController extends Controller
{
    ...
    public function __construct()
    {
        ...
        // Page Title
        $this->module_title = 'notification.email_templates';
    ...
    public function index()
    {
        ...
        $customefield = CustomField::exportCustomFields(new NotificationTemplate());
        $export_import = true;
        $export_columns = [
            [
                'value' => 'id',
                'text' => __('notification.lbl_id'),
            ],
            [
                'value' => 'label',
                'text' => __('notification.lbl_label'),
            ],

        ];
        $export_url = route('backend.notificationtemplates.export');
        return view('notificationtemplate::backend.notificationtemplates.index_datatable', compact('module_action','filter', 'columns', 'customefield', 'export_import', 'export_columns', 'export_url'));
    }
    ...
    public function index_data(Datatables $datatable)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->orderColumn('label', function ($query, $order) {
                $query->join('notification_template_content_mapping', 'notification_templates.id', '=', 'notification_template_content_mapping.template_id')
